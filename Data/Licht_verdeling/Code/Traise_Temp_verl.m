...
for i = 1: length(L);
    for j=1:length(NA)
        ...
        psiplus=csvread(fullfile(path,'UnitFR',[protocol_name '+u_a']));
        psiminus=csvread(fullfile(path,'UnitFR',[protocol_name '-u_a']));
        ...
        if j==1
            for l=1:length(t1)
                ...
                dTplus(k,l)=dTError(phiplus(2,500),du_a_plus(i),...
                    du_s_minus(i),prop,t1(l));
                dTminus(k,l)=dTError(phiminus(2,500),du_a_minus(i),...
