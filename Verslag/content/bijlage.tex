...
De bestanden voor de verschillende protocols zijn als volgt gelabeld:
...
\label{ssub:dtcontinue}
Onderstaande matlabcode leest de .csv bestanden uit de map \emph{unitFR} en berekent de temperatuursverandering voor een gepulste stimulatie op de manier zoals uitgelegd in paragraaf \ref{sub:temperatuursverandering_simulatie}. Vervolgens wordt de temperatuurstijging op positie [(pos($1$), pos($2$)] geplot in functie van de tijd voor de vier stimulatie protocols.\\ 
\textbf{Puls$\_$plot.m}
\begin{lstlisting}
%-------Setup-------
Puls_Setup_Data
...
\begin{figure*}[!ht]
	...
	\caption{Temperatuurstijging voor continue stimulatie bij $474nm$, SM en MM en bij $560nm$, MM op 3 verschillende tijdstippen met een input vermogen van $1mW$.}
...
\begin{figure*}[!ht]
	...
	\caption{De invloed van $\mu_a \pm \sigma$ en $\mu_s \pm \sigma$ op de lichtverdeling voor $474nm$, SM en MM en voor $560nm$, SM met een input vermogen van $1mW$.}
	\label{fig:FR_scat+abs_variatie}
\end{figure*}
\begin{figure*}[!ht]
    \centering
    \includegraphics[width=0.8\textwidth]{gfx/dT_time_mu-a_mu-sSM.png}
    \caption{De invloed van $\mu_a \pm \sigma$ en $\mu_s \pm \sigma$ op de temperatuurstijging voor SM voor een input vermogen van $1mW$.}
    \label{fig:foutSM}
\end{figure*}
