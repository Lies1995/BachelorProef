...
\begin{figure*}[!ht]
  ...
  \end{figure*} 
\\
Op figuren \ref{fig:dT_tijd_SM}  en \ref{fig:dT_tijd_MM} werd de temperatuurstijging van het hersenweefsel geplot in functie van de tijd op $z=0$ en $r=0$, telkens voor de twee verschillende golflengtes. Op de grafieken werd telkens $t=\tau$ aangeduid met een stippellijn. Aangezien de tijdsconstante voor eenzelfde vezel (SM of MM) zeer weinig van elkaar verschillen (zie later) worden de resultaten voor een zelfde vezel maar verschillende golflengte op \'e\'en afbeelding afgebeeld. Wanneer men naar de vorm van de grafieken kijkt, is te zien dat de temperatuurstijging inderdaad vrij snel zal satureren. Anderzijds is het opvallend dat de temperatuurstijging voor $560nm$ veel groter is dan voor $474nm$. Dit komt omdat de absorptie co\"ffici\"ent bij $560nm$ groter is dan bij $474nm$, door absorptie door hemoglobin. Aangezien absorptie voornamelijk verantwoordelijk is voor temperatuurstijging, verklaart dit de grotere temperatuursverandering bij $560nm$.
%figuur
...
\begin{figure}[!ht]
  ...
  \end{figure} 
	...
	\begin{figure}[!ht]
  	...
  	\label{fig:dT_tijd_MM}
  \end{figure} 
Zoals daarnet al werd vermeld, is er een zeer klein verschil tussen de tijdsconstanten voor eenzelfde vezel. Dit kan men zien in tabel \ref{tab:tau}. De reden hiervoor is dat de waarde van de optische parameters (deze zijn afhankelijk van de golflengte) weinig invloed heeft op $\tau$. Ze heffen elkaar als het ware op. Anderzijds heeft de gebruikte vezel een zeer grote invloed op de waarde van $\tau$. De rede hiervoor is dat de tijdsconstante zeer sterk afhankelijk is van de straal van de optische vezel ($w_L$). Men kan zien dat voor de muli-mode vezel de tijdsconstanten veel groter zijn dan voor single-mode. Een SM vezel is smaller, waardoor alle energie zich in een klein en smal gebied bevindt. Hierdoor kan de temperatuur makkelijker en dus sneller stijgen, wat leidt tot een kleine $\tau$. Voor de MM vezel is dezelfde energie over een groter gebied verspreidt, waardoor de temperatuur trager zal stijgen. Dit geeft een grotere waarde voor de tijdsconstante. 
\begin{figure*}[!ht]
    \centering
...
\begin{figure*}[!ht]
  ...
  \end{figure*} 
Als eerste bekijken we de invloed van de weefselparameters $\mu_a$ en $\mu_s$ op de lichtverdeling. De parameters worden gevari\"eerd binnen hun foutenmarges. 
Dit wordt afgebeeld op figuur \ref{fig:scat+abs_variatie} voor $560nm$ en MM. De figuren voor de andere protocols kan men vinden in de bijlage op figuur \ref{fig:FR_scat+abs_variatie}. Op deze figuren is te zien dat de absorptieco\"effici\"ent voornamelijk een invloed heeft op de grootte van de contour en dat de verstrooiingsco\"effici\"ent vooral een invloed heeft op de vorm van de contour. Een grotere absorptie zorgt ervoor dat de energie gemakkellijker geabsorbeerd wordt en dus minder verspreid wordt. Het gestimuleerde volume wordt kleiner. Een grotere verstrooiing zorgt ervoor dat de energie meer verspreid wordt in de radiale richting.  

De fout op de temperatuurstijging werd als volgt berekend: De twee parameters waarop fouten gekend zijn ($\mu_a$ en $\mu_s$), werden opnieuw elk afzonderlijk gevarieerd binnen hun foutenmarges. Door telkens de temperatuurstijging te berekenen met $\mu_a \pm \sigma$ of $\mu_s \pm \sigma$ kon achterhaald worden wat de invloed was van elke parameter op de temperatuurstijging.  Dit kan men zien op figuur \ref{fig:foutMM} voor multi-mode en voor single-mode in de bijlage (figuur \ref{fig:foutSM}). Uit deze analyse blijkt dat de absorptieco\"effici\"ent een zeer grote invloed heeft op $\Delta T$, namelijk een grotere absorptie zorgt voor een grotere temperatuurstijging en omgekeerd, een kleinere absorptie zorgt voor een kleinere temperatuurstijging. De verstrooiingsco\"effici\"ent blijkt zeer weinig invloed te hebben op de grootte van de temperatuurstijging. Toch is te zien dat een grotere verstrooiing zorgt voor een iets grotere temperatuurstijging en dat een kleinere verstrooiing een iets kleinere temperatuurstijging tot gevolg heeft. 
...
\begin{figure*}[!ht]
    ...
    \label{fig:foutMM}
  \end{figure*}  
Vervolgens wordt $\Delta T$ opnieuw berekend, met $\mu_a + \sigma$ en $\mu_s + \sigma$ om als bovengrens van de fout op $\Delta T$ te gebruiken en met $\mu_a - \sigma$ en $\mu_s - \sigma$ om de ondergrens van de fout op $\Delta T$ te gebruiken. Dit staat afgebeeld in figuren \ref{fig:dT_tijd_SM} en \ref{fig:dT_tijd_MM}. De fout op de temperatuurstijging wordt enkel berekend voor continue stimulatie. Aangezien gepulste stimulatie vaker gebruikt wordt, is het zeer interessant om ook hier de fouten op te berekenen. Dat werd echter nog niet gedaan. \\
De fouten op de temperatuursverandering kunnen ook op een meer wiskundige manier berekend worden, namelijk met de doorwerkingsregel. Hiervoor is een kwantitatieve fout nodig op de lichtverdeling. Omdat dit moeilijk te kwantifici\"eren valt, worden als foutenmarges voor de lichtverdeling $\phi^+(r,z)$ en $\phi^-(r,z)$ gebruikt,  respectivelijk als bovengrens en ondergrens (zie paragraaf \ref{sub:energieverdeling}). \\
 Het resultaat van deze foutenberekening is te zien in figuur \ref{fig:FRFouteFoutMM}. Zoals men kan zien is de fout exponentieel afhankelijk van de tijd. Hierdoor gaat $\Delta T - \sigma$ na een bepaalde tijd onder $0$. Dit is niet fysisch en daarom werd geopteerd voor de eerste methode.
	\begin{figure}[]
  	\centering
  	\includegraphics[width=0.5\textwidth]{gfx/FRFouteFoutMM.png}
  	\caption{Temperatuurstijging met fouten op $z=0$ en $r=0$ voor MM}
  	\label{fig:FRFouteFoutMM}
  \end{figure}   

De fout op $\tau$ berekend als volgt: 
\begin{equation}
